...
import com.jfservice.sys.client.handler.alpha.AlphaAdultHandler;
...
@Controller
@Scope("prototype")
public class ImmediationSerHandlerAlp extends IoHandlerAdapter {

	private final static String TAG = ImmediationSerHandlerAlp.class.getSimpleName();
	private Logger logger = Logger.getLogger(ImmediationSerHandlerAlp.class);
	/**
	 * 设备端和客户端
	 */
	...
	private final static String ALPHA_T_D = "ALPHA";          //阿尔法青少年--设备端
	...
	@Autowired
	private AlphaAdultHandler alphaAdultHandler;//青少年设备
	...
	public void messageReceived(IoSession session,Object message) throws Exception{		
		...
		if(temp.getContent().contains(ALPHA)){
			notification.setEventHandler(alphaHandler);
		}else if(temp.getContent().contains(ALPHA_T_D)){
			notification.setEventHandler(alphaAdultHandler);
		}
	...
	public void messageSent(IoSession session, Object message) {
		logger.info("发送成功="+session.getId()+"发送内容="+((DataBean)message).getContent());
	}
	@Override
